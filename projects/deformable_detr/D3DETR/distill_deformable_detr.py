...
class DistillDeformableDETR(DeformableDETR):
    ...
    def forward_student(self, images, img_masks,teacher_query):
        ...
        output = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord[-1],"self_attn_maps":self_attn_maps[-1],"cross_attn_maps":cross_attn_maps[-1]}
        if self.aux_loss:
            output["aux_outputs"] = self._set_aux_loss2(outputs_class, outputs_coord,self_attn_maps,cross_attn_maps)
        ...
        output['teacher_predictions'] = {
            ...
            "self_attn_maps":self_attn_maps_tea[-1],
            "cross_attn_maps":cross_attn_maps_tea[-1],
            "aux_outputs": self._set_aux_loss2(outputs_class_tea, outputs_coord_tea,self_attn_maps_tea,cross_attn_maps_tea)
        }
        return output
    ...
    @torch.jit.unused
    def _set_aux_loss2(self, outputs_class, outputs_coord,self_attn_maps,cross_attn_maps):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [
            {"pred_logits": a, "pred_boxes": b,"self_attn_maps":c,"cross_attn_maps":d}
            for a, b,c,d in zip(outputs_class[:-1], outputs_coord[:-1],self_attn_maps[:-1],cross_attn_maps[:-1])
        ]

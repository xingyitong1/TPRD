...
def batch_box_iou(boxes1, boxes2):
    ...
    Args:
        boxes1: (torch.Tensor[B, N, 4]): First set of bounding boxes, B is batch size, N is number of boxes per batch
        boxes2: (torch.Tensor[B, M, 4]): Second set of bounding boxes, B is batch size, M is number of boxes per batch
    ...
    # Calculate area while keeping batch dimension
...
class DistillCriterion(SetCriterion):
    ...
    def loss_labels_distill(self, outputs, outputs_t, indices, num_boxes, weights=None,decoupled = False,targets = None):
        ...
        if not decoupled:
            ...
            alpha = 1.0
            beta = 1.0  
            loss_ce[idx] = (alpha * tckd + beta * nckd) * (1 ** 2) # temperature = 1 here
    ...
    def optimize_single_stage(self, all_stages_output, current_stage_idx, targets, indices):
        ...
        # Prepare predictions from all stages
        ...
        for b in range(batch_size):
            ...
            if len(src_idx) > 0:
                ...
                # Get current stage score as baseline

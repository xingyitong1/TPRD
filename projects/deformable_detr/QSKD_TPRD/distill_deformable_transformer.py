...
from ..modeling.deformable_transformer import DeformableDetrTransformerDecoder
from .attention import DistillMultiScaleDeformableAttention
from detrex.layers import MultiheadAttention
from .transformer import DistillBaseTransformerLayer
...
class DistillDeformableDetrTransformerDecoder(TransformerLayerSequence):
    def __init__(
        ...
        super(DistillDeformableDetrTransformerDecoder, self).__init__(
            transformer_layers=DistillBaseTransformerLayer(
                attn=[
                    MultiheadAttention(
                        embed_dim=embed_dim,
                        num_heads=num_heads,
                    ...
                    DistillMultiScaleDeformableAttention(
                        embed_dim=embed_dim,
                        num_heads=num_heads,
    ...
    def forward_distill(
        ...
        **kwargs,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            output,cross_attn_map = layer.forward_distill(
                output,
                key,
                value,
                query_pos=query_pos,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points),torch.stack(cross_attn_maps)
...
class DistillDeformableDetrTransformer(DeformableDetrTransformer):
    ...
    def forward_teacher(
        ...
        **kwargs,
        ...
        # decoder
        inter_states, inter_references,cross_attn_maps = self.decoder.forward_distill(
            query=query,  # bs, num_queries, embed_dims
            key=None,  # bs, num_tokens, embed_dims
            value=memory,  # bs, num_tokens, embed_dims
            query_pos=query_pos,
        ...
        return inter_states, init_reference_out, inter_references_out,None ,None,cross_attn_maps,memory
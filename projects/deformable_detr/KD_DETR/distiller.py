...
class Distiller(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        # TODO general sampling points described in paper
        # 每一次iteration都会重新sample，不是可学习的参数
        aux_query = torch.nn.Embedding(self.general_points,self.student.transformer.embed_dim * 2, device=self.device).weight
        # original features
        with torch.no_grad():
            output_t,teacher_query = self.teacher.forward_teacher(images,img_masks,aux_query)
        output_s = self.student.forward_student(images,img_masks,aux_query,teacher_query) 

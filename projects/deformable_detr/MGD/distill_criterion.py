...
class DistillCriterion(SetCriterion):
    ...
    """
    def __init__(self,mask_ratio,dim_in,dim_out,num_features, *args,**kwargs):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, weights=None):
        """
        Compute KL divergence loss between student and teacher network outputs
        """
    ...
    def get_dis_loss(self, preds_S, preds_T,index):
        ...
        new_fea = self.generation[index](masked_fea)
    ...
    def forward(self, outputs_s, outputs_t,targets):
        ...
        for i, (features_s, features_t) in enumerate(zip(outputs_s["multi_level_features"], outputs_t["multi_level_features"])):
            # Calculate discriminative score of teacher model
            loss_masked += self.get_dis_loss(features_s,features_t,i)
        losses["loss_distill_masked_neck"] = loss_masked / len(outputs_s["multi_level_features"])

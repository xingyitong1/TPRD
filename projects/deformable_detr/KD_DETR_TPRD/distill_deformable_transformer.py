...
class DistillDeformableDetrTransformer(DeformableDetrTransformer):
    ...
    def forward_teacher(
        ...
        **kwargs,
        ...
        # TODO 拼接auxilary_query
        auxiliary_query_pos,auxiliary_query = torch.split(auxiliary_query, c, dim=1)
        auxiliary_query_pos = auxiliary_query_pos.unsqueeze(0).expand(bs, -1, -1)
        auxiliary_query = auxiliary_query.unsqueeze(0).expand(bs, -1, -1)
        auxiliary_reference_points = self.reference_points(auxiliary_query_pos).sigmoid()
        ...
        # aux query
        inter_states_aux, inter_references_aux = self.decoder(
            query=auxiliary_query,  # bs, num_queries, embed_dims
            key=None,  # bs, num_tokens, embed_dims
            value=memory,  # bs, num_tokens, embed_dims
            query_pos=auxiliary_query_pos,
            key_padding_mask=mask_flatten,  # bs, num_tokens
            reference_points=auxiliary_reference_points,  # num_queries, 4
            spatial_shapes=spatial_shapes,  # nlvl, 2
            level_start_index=level_start_index,  # nlvl
            valid_ratios=valid_ratios,  # bs, nlvl, 2
            **kwargs,
        ...
        return inter_states, init_reference_out, inter_references_out, inter_states_aux,auxiliary_reference_points ,inter_references_aux,None ,None,(teacher_query_pos, teacher_query,teacher_refpoints)
    ...
    def forward_student(
        ...
        **kwargs,
        ...
        return inter_states, init_reference_out, inter_references_out,inter_states_aux,auxiliary_reference_points,inter_references_aux,inter_states_tea,teacher_refpoints,inter_references_tea,None,None

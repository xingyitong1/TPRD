...
class Distiller(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        memory_t = self.teacher.forward_encoder(images,img_masks)[0]
        memory_s,mask_flatten,spatial_shapes,level_start_index,valid_ratios = self.student.forward_encoder(images,img_masks)

        output_t = self.teacher.forward_decoder_teacher(memory_t,memory_s,mask_flatten,spatial_shapes,level_start_index,valid_ratios)
        
        output_s = self.student.forward_decoder_student(memory_s,mask_flatten,spatial_shapes,level_start_index,valid_ratios)

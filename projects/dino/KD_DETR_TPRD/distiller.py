...
class Distiller(nn.Module):
    ...
    def __init__(
        ...
        # number of general sampling points
    ...
    def forward(self, batched_inputs):
        ...
        aux_target = torch.nn.Embedding(self.general_points,self.student.transformer.embed_dim, device=self.device).weight
        aux_reference_points = torch.nn.Embedding(self.general_points,4,device=self.device).weight
        aux_query = (aux_target,aux_reference_points)
        
        gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
        targets = self.prepare_targets(gt_instances)
        # original features
        with torch.no_grad():
            output_t,teacher_query = self.teacher.forward_teacher(images,img_masks,aux_query)
        output_s = self.student.forward_student(images,img_masks,targets,aux_query,teacher_query) 

        # student loss
        loss_dict = self.student.criterion(output_s, targets,output_s["dn_meta"])
        # distill loss
        loss_dict.update(self.distill_criterion(output_s, output_t,targets,self.event_storage.iter))
        
        for k in loss_dict.keys():
            if k in self.student.criterion.weight_dict:
                loss_dict[k] *= self.student.criterion.weight_dict[k]
            elif k in self.distill_criterion.weight_dict:

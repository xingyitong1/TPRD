...
def batch_box_iou(boxes1, boxes2):
    """Batch IoU calculation function
    ...
    Returns:
        Tuple: A tuple containing pairwise IoU and union, with shape (torch.Tensor[B, N, M], torch.Tensor[B, N, M])
    """
    B, N = boxes1.shape[:2]
    M = boxes2.shape[1]
    
    # Calculate area, keep batch dimension
...
class DistillCriterion(SetCriterion):
    ...
    def __init__(self, temperature=10.0,drop_iter=300000, **kwargs):
    ...
    def _normalize_weights(self, weights, dim=-1):
        """
        Auxilary function to normalize weights
        """
    ...
    def _weighted_mean(self, loss, weights, dim=1):
        """
        Auxilary function to calculate weighted mean loss
        """
        if weights is not None:
    ...
    # Overwrite loss_labels because KL divergence loss is used here
    ...
    def optimize_single_stage(self, all_stages_output, current_stage_idx, targets, indices):
        ...
        # Pre-prepare predictions of all stages
        ...
        # Get scores and classes of the current stage as baseline
        ...
        for b in range(batch_size):
            ...
            if len(src_idx) > 0:
                ...
                # Calculate classification scores [num_stages, num_positive]
                ...
                # Calculate IoU of previous stages
                ...
                # Get scores of the current stage as baseline
            ...
            if len(negative_indices) > 0:
                ...
                if valid_updates.any():  # Only operate when there are samples that need updates
    ...
    def forward(self, outputs_s, outputs_t,targets,iter):
